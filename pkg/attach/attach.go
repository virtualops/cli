...
// We use a slightly customised version of k8s.io/kubectl/pkg/cmd/attach.go
// due to a problem with the RESTClient, see L132 for comment.
...
func DefaultAttachFunc(o *AttachOptions, containerToAttach *corev1.Container, raw bool, sizeQueue remotecommand.TerminalSizeQueue) func() error {
	return func() error {
		//restClient, err := restclient.RESTClientFor(o.Config) <-- this doesn't work because o.Config.GroupVersion == nil
		restClient, err := o.restClientGetter.(cmdutil.Factory).RESTClient()
		if err != nil {
			return err
		}
		req := restClient.Post().
